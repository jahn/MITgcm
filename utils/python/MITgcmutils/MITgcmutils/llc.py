...
def _pcolormesh(x, y, d, **kwargs):
    ...
    if len(i):
        ny, nx = d.shape
        d[np.clip(j,0,ny-1),np.clip(i,0,nx-1)] = np.NaN
        d[np.clip(j,0,ny-1),np.clip(i-1,0,nx-1)] = np.NaN
        d[np.clip(j-1,0,ny-1),np.clip(i,0,nx-1)] = np.NaN
        d[np.clip(j-1,0,ny-1),np.clip(i-1,0,nx-1)] = np.NaN
        x[j,i] = 0
        y[j,i] = 0
